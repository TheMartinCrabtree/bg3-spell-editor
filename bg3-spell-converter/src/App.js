...
const App = () => {
  ...
  jsonObject && console.log("typeof", typeof jsonObject);

  const _renderSpells = () => {
    return (
      jsonObject &&
      jsonObject.map((spellData, index) => (
        <SpellDisplay spellData={spellData} key={index}>
          test
        </SpellDisplay>
      ))
    );
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "Arial" }}>
      ...
      <h2>Filter over level 6</h2>
      <button onClick={_filterOver6}>Filter Spells</button>
